...
*Setup instructions for a gitian build of Dach using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Dach
...
Clone the git repositories for dach and gitian and then checkout the dach version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/dach-crypto/dach.git
cd dach
...
Follow the instructions in [doc/release-process.md](release-process.md) in the dach repository
...
To build Dach (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the dach repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/dach/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/dach-crypto/dach
...
For example:
```bash
URL=https://github.com/crowning-/dach.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit dach=${COMMIT} --url dach=${URL} ../dach/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit dach=${COMMIT} --url dach=${URL} ../dach/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit dach=${COMMIT} --url dach=${URL} ../dach/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/dach-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/dach-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/dach-build.assert
```
...
[dach/gitian.sigs](https://github.com/dach-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
