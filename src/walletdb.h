...
public:
    ...
    bool WriteZDACHXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDACHXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDACHXSeed_deprecated(uint256& seed);
    bool EraseZDACHXSeed();
    bool EraseZDACHXSeed_deprecated();

    bool WriteZDACHXCount(const uint32_t& nCount);
    bool ReadZDACHXCount(uint32_t& nCount);
