...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DACHX);
    ...
    if (!settings.contains("nAnonymizeDachAmount"))
        settings.setValue("nAnonymizeDachAmount", 1000);

    nAnonymizeDachAmount = settings.value("nAnonymizeDachAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDachAmount"))
        SoftSetArg("-anonymizedachamount", settings.value("nAnonymizeDachAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in dach.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDachAmount:
            return QVariant(nAnonymizeDachAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDachAmount:
            nAnonymizeDachAmount = value.toInt();
            settings.setValue("nAnonymizeDachAmount", nAnonymizeDachAmount);
            emit anonymizeDachAmountChanged(nAnonymizeDachAmount);
            break;
        case CoinControlFeatures:
