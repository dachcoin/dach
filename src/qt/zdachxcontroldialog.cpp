...
#include "ui_zdachxcontroldialog.h"
...
std::set<std::string> ZDachxControlDialog::setSelectedMints;
std::set<CMintMeta> ZDachxControlDialog::setMints;

ZDachxControlDialog::ZDachxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDachxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDachxControlDialog::~ZDachxControlDialog()
{
    delete ui;
}

void ZDachxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDachxControlDialog::updateList()
{
...
void ZDachxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDachxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDachx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDachxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDachxControlDialog::GetSelectedMints()
{
...
void ZDachxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
