...
// Copyright (c) 2019 The Dachcoin developers
...
#if defined(NDEBUG)
#error "Dach cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in udachx) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zDACHX is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zDACHX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zDACHX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zDACHX spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zDACHX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zDACHX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdachx");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zDACHX mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        ...
        nSubsidy = 2 * COIN;
    } else if (nHeight < 40000 && nHeight >= 800) {
        nSubsidy = 0.1 * COIN;
	} else if (nHeight < 48509 && nHeight >= 40000) {
        nSubsidy = 40 * COIN;
	} else if (nHeight < 141710 && nHeight >= 48509) {
        nSubsidy = 30 * COIN;
	} else if (nHeight < 341711 && nHeight >= 141710) {
        nSubsidy = 20 * COIN;
	} else if (nHeight < 1041712 && nHeight >= 341711) {
        nSubsidy = 10 * COIN;
	} else if (nHeight < 1741713 && nHeight >= 1041712) {
        nSubsidy = 5 * COIN;
	} else if (nHeight < 4621766 && nHeight >= 1741713) {
        nSubsidy = 2.5 * COIN;
	} else {
        nSubsidy = 1.5 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZDACHXStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Dach
...
{
    RenameThread("dach-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZDACHXMinted()
{
...
void RecalculateZDACHXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zDACHX supply
...
bool RecalculateDACHXSupply(int nHeightStart)
{
...
bool UpdateZDACHXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateDACHXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zDACHX money supply in the block index
    if (!UpdateZDACHXSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zDACHX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zDachxSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert DACHX to zDACHX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zDACHX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zDACHX serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zDACHX stake block is less than allowed start height", __func__);

    if (CZDachxStake* zDACHX = dynamic_cast<CZDachxStake*>(stake)) {
        CBlockIndex* pindexFrom = zDACHX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zDACHX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zDACHX stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zDACHX->GetValue()));
        if (nChecksum200 != zDACHX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zDACHX->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZDACHX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zDACHX fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zDACHX mints and %d zDACHX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Dach: We use certain sporks during IBD, so check to see if they are
