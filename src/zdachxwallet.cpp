...
CzDACHXWallet::CzDACHXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdachx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDACHXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDACHXSeed_deprecated()) {
                    LogPrintf("%s: Updated zDACHX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdachx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdachx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDACHXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDACHXCount(nCountLastUsed);
    else if (!walletdb.ReadZDACHXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDACHXWallet::Lock()
{
    seedMaster = 0;
}

void CzDACHXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDACHXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDACHX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDACHXWallet::LoadMintPoolFromDB()
{
...
void CzDACHXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDACHXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDACHXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdachxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDACHXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDACHX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdachxTracker which also adds to database
    pwalletMain->zdachxTracker->Add(dMint, true);
...
void CzDACHXWallet::SeedToZDACHX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDACHXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDACHXWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZDACHXCount(nCountLastUsed);
...
void CzDACHXWallet::GenerateDeterministicZDACHX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDACHXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDACHXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
