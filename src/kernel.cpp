...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZDachxStake(spend));
    } else {
        ...
        CDachxStake* dachxInput = new CDachxStake();
        dachxInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(dachxInput);
    }
