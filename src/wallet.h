...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DACHX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDACHX send
enum ZerocoinSpendStatus {
    ZDACHX_SPEND_OKAY = 0,                            // No error
    ZDACHX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDACHX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDACHX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDACHX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDACHX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDACHX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDACHX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDACHX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDACHX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDACHX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDACHX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDACHX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDACHX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDACHX_SPENT_USED_ZDACHX = 14,                      // Coin has already been spend
    ZDACHX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZDACHX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZDachxBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDACHXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzdachxAuto) const;
    ...
    */
    ...
    CzDACHXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDACHXTracker> zdachxTracker;
    ...
    void setZWallet(CzDACHXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zdachxTracker = std::unique_ptr<CzDACHXTracker>(new CzDACHXTracker(strWalletFile));
    }

    CzDACHXWallet* getZWallet() { return zwalletMain; }
    ...
    void setZDachxAutoBackups(bool fEnabled)
    {
    ...
    /** zDACHX reset */
    boost::signals2::signal<void()> NotifyzDACHXReset;
