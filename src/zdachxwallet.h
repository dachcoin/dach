...
#ifndef DACH_ZDACHXWALLET_H
#define DACH_ZDACHXWALLET_H
...
public:
    CzDACHXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDACHX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDACHX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DACH_ZDACHXWALLET_H
