...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zdachxTracker = unique_ptr<CzDACHXTracker>(new CzDACHXTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZDACHX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZDACHX_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
